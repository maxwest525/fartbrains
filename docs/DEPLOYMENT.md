...
### `ALLOWED_ORIGIN` is set — two functions still answer `*`

Set on 2026-09-06. Re-verified against production on 2026-09-07: every function
that goes through `supabase/functions/_shared/cors.ts` now echoes the origin
instead of `*`.

```
$ curl -s -X OPTIONS https://uwuhfvhqnpozhndrabwl.supabase.co/functions/v1/generate-prompt \
    -H 'Origin: https://evil.example' -D - -o /dev/null | grep -i allow-origin
access-control-allow-origin: https://fartbrain.app
```

Spot-checked the same way and pinned: `create-checkout-session`,
`create-portal-session`, `summarize`, `ash-chat`, `scrape-url`, `extract-url`,
`notes-feed`, `delete-account`, `resolve-share`. Server-to-server functions
(`stripe-webhook`, `process-email-queue`, `handle-email-suppression`) send no
CORS header at all, which is correct — nothing calls them from a browser.

**Two functions are still `*`,** because neither imports the shared helper:

| Function | Why it is not pinned | Risk |
|---|---|---|
| `mcp` | Generated bundle (`functions/mcp/index.ts`), regenerated by Lovable's Vite plugin from `src/lib/mcp/**` — the shared CORS module is not part of what it emits. | Authenticated by OAuth token rather than the browser session, so a page cannot ride a logged-in customer's cookie into it. Still worth pinning. |
| `push-public-key` | Hand-written, `verify_jwt = false`. | Returns the VAPID public key, which is public by definition. |

The other two on the MISS list (`process-email-queue`,
`handle-email-suppression`) are not browser-reachable and need no header.

The `*` fallback exists so local development and preview deploys keep working;
production overrides it via the secret.
