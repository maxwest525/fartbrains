...
const chapters = ["Capture", "Extract", "Connect", "Synthesize", "Build", "Ash"];
...
function ThoughtWorld({ chapter, pulse, density }: WorldProps) {
  ...
  useEffect(() => {
    ...
    const points = new THREE.Points(
      geometry,
      new THREE.PointsMaterial({ size: 0.105, vertexColors: true, transparent: true, opacity: 0.92, blending: THREE.AdditiveBlending, depthWrite: false }),
    );
    world.add(points);
    ...
    for (let i = 0; i < nodeCount; i++) {
      for (const step of [3, 13]) {
        ...
        edgePositions.push(base[i * 3], base[i * 3 + 1], base[i * 3 + 2], base[j * 3], base[j * 3 + 1], base[j * 3 + 2]);
      }
    }
    const lines = new THREE.LineSegments(
      new THREE.BufferGeometry().setAttribute("position", new THREE.Float32BufferAttribute(edgePositions, 3)),
      new THREE.LineBasicMaterial({ color: 0x8567ee, transparent: true, opacity: 0.11, blending: THREE.AdditiveBlending }),
    );
    world.add(lines);

    const core = new THREE.Mesh(
      new THREE.IcosahedronGeometry(0.72, 1),
      new THREE.MeshBasicMaterial({ color: 0x9d7cff, wireframe: true, transparent: true, opacity: 0.88 }),
    );
    world.add(core);
    ...
    for (let r = 0; r < 4; r++) {
      ...
      for (let n = 0; n <= 160; n++) {
        ...
        curve.push(new THREE.Vector3(Math.cos(a) * (6.4 + r * 0.34), Math.sin(a) * (6.4 + r * 0.34), 0));
      }
      const ring = new THREE.Line(
        new THREE.BufferGeometry().setFromPoints(curve),
        new THREE.LineBasicMaterial({ color: r === 0 ? 0x35d8ff : 0x684bbd, transparent: true, opacity: r === 0 ? 0.23 : 0.1 }),
      );
    ...
    for (let i = 0; i < 900; i++) {
      dustPositions.set([(Math.random() - 0.5) * 70, (Math.random() - 0.5) * 42, -12 + Math.random() * 18], i * 3);
    }
    const dust = new THREE.Points(
      new THREE.BufferGeometry().setAttribute("position", new THREE.BufferAttribute(dustPositions, 3)),
      new THREE.PointsMaterial({ color: 0x6a58a3, size: 0.035, transparent: true, opacity: 0.45 }),
    );
    scene.add(dust);
    ...
    const draw = () => {
      ...
      world.scale.lerp(new THREE.Vector3(targetScale, targetScale, targetScale), 0.035);
      ...
      camera.position.z += ((innerWidth < 720 ? 27 : 24) + c * 1.05 - camera.position.z) * 0.028;
  ...
  return <canvas ref={canvasRef} className="fb-world" aria-label="Interactive three-dimensional map of captured and connected ideas" />;
}

function CaptureTerminal({ open, onClose }: { open: boolean; onClose: () => void }) {
  const [phase, setPhase] = useState(0);
  useEffect(() => {
    if (!open) return;
    setPhase(0);
    const timers = [700, 1500, 2400, 3400].map((ms, i) => window.setTimeout(() => setPhase(i + 1), ms));
    return () => timers.forEach(clearTimeout);
  }, [open]);
  if (!open) return null;
  const states = ["Reading source", "Transcribing 00:47", "Researching 12 sources", "Connecting 7 notes", "Build package ready"];
  return (
    <div className="fb-terminal-backdrop" role="dialog" aria-modal="true" aria-label="Live Fartbrain capture simulation">
      <div className="fb-terminal">
        <header><b>FARTBRAIN / LIVE CAPTURE</b><span>SIMULATION</span><button onClick={onClose}>CLOSE ×</button></header>
        <div className="fb-terminal-grid">
          <div className="fb-terminal-source"><small>SOURCE / INSTAGRAM REEL</small><h3>Competitor keywords for cheaper paid acquisition</h3><p>“Use this same signal for SEO. Decide whether every opportunity should become an ad, a page, or both.”</p></div>
          <div className="fb-terminal-state"><small>PIPELINE STATUS</small><strong>{states[phase]}</strong><div>{states.map((state, i) => <span key={state} className={i <= phase ? "done" : ""}>{String(i + 1).padStart(2, "0")} {state}</span>)}</div></div>
        </div>
        <div className={`fb-terminal-result ${phase === 4 ? "visible" : ""}`}><small>BUILD READY</small><h2>Competitor Signal Engine</h2><div><span>PRODUCT BRIEF</span><span>RESEARCH PACK</span><span>MVP SPEC</span><span>BUILD PROMPT</span></div></div>
      </div>
    </div>
  );
}

export default function LandingCinematic({ onEnter }: { onEnter?: () => void }) {
  ...
  useEffect(() => {
    ...
    document.title = "Fartbrain — turn fleeting thoughts into things you can build";
    const onScroll = () => {
    ...
    return () => {
      ...
      document.documentElement.classList.remove("fb-cinema-route", "route-wide");
      document.title = oldTitle;
    };
  }, []);

  const sectionRef = useCallback((index: number) => (node: HTMLElement | null) => { sections.current[index] = node; }, []);
  const run = () => { setPulse((v) => v + 1); setTerminal(true); };
  ...
  return (
    <div className="fbc">
      ...
      <nav className="fb-cinema-nav">
        <a href="#capture" className="fb-wordmark"><i>◫</i> FARTBRAIN</a>
        <div>{chapters.map((name, i) => <a key={name} className={chapter === i ? "active" : ""} href={`#${name.toLowerCase()}`}>{String(i + 1).padStart(2, "0")}</a>)}</div>
        <button onClick={() => onEnter?.()}>OPEN BRAIN ↗</button>
      </nav>

      <aside className="fb-global-status"><span>LIVE THOUGHT SYSTEM</span><b>{chapters[chapter]}</b></aside>

      <main>
        <section id="capture" ref={sectionRef(0)} className="fb-chapter fb-opening">
          <div className="fb-copy">
            <span className="fb-eyebrow">01 / THE THOUGHT BEFORE IT DISAPPEARS</span>
            <h1>Don&rsquo;t save<br />the post.<br /><em>Save what it<br />could become.</em></h1>
            <p>A reel. A URL. Nine words you typed before the thought vanished.</p>
            <button className="fb-primary" onClick={run}>Capture this idea <span>↗</span></button>
            <small>RUN THE LIVE SYSTEM · 00:05</small>
          </div>
          <div className="fb-capture-card"><small>INCOMING / INSTAGRAM REEL</small><strong>Steal competitor keywords for lower-cost paid ads.</strong><p>YOUR TWEAK</p><b>“Use it for SEO too. Make it one system.”</b></div>
          <footer><span>ONE FLEETING THOUGHT.</span><a href="#extract">SCROLL TO OPEN IT ↓</a><span>01 — 06</span></footer>
        </section>

        <section id="extract" ref={sectionRef(1)} className="fb-chapter fb-extract">
          <div className="fb-copy"><span className="fb-eyebrow">02 / NOTHING USEFUL GETS LEFT BEHIND</span><h2>The link<br /><em>opens up.</em></h2><p>Transcript. Summary. Claims. Sources. Every useful URL hiding underneath the original thought.</p></div>
          <div className="fb-extraction-stack"><div><span>01</span><b>TRANSCRIPT</b><small>47 seconds → searchable text</small></div><div><span>02</span><b>SUMMARY</b><small>strategy, sequence, why it works</small></div><div><span>03</span><b>DEEP RESEARCH</b><small>12 corroborating sources</small></div><div><span>04</span><b>URL EXTRACTION</b><small>tools, references, datasets</small></div></div>
        </section>

        <section id="connect" ref={sectionRef(2)} className="fb-chapter fb-connect">
          <div className="fb-copy"><span className="fb-eyebrow">03 / YOUR BRAIN IS NOT A FOLDER TREE</span><h2>Old thoughts<br />come back <em>alive.</em></h2><p>Metadata gives every note context. Fartbrain surfaces the relationships you would never remember to search for.</p><label>RELATIONSHIP DENSITY <b>{density}%</b><input type="range" min="12" max="100" value={density} onChange={(e) => setDensity(Number(e.target.value))} /></label></div>
          <div className="fb-related"><article><small>3 MONTHS AGO · PAID ADS</small><b>Competitor keyword gaps expose underpriced intent.</b></article><article><small>6 WEEKS AGO · SEO</small><b>Adjacent rankings are the fastest pages to expand.</b></article><article><small>TODAY · YOUR TWEAK</small><b>One engine decides: ad, page, or both.</b></article><div><span>7 NOTES FOUND</span><strong>1 NEW THROUGH-LINE</strong></div></div>
        </section>

        <section id="synthesize" ref={sectionRef(3)} className="fb-chapter fb-synthesize">
          <div className="fb-copy"><span className="fb-eyebrow">04 / ASH</span><h2>Talk until<br />the idea <em>clicks.</em></h2><p>Ash enters with the source, research, your note and everything related already in context. No cold-start chatbot.</p><button className="fb-ghost" onClick={run}>Open this conversation ↗</button></div>
          <div className="fb-ash"><header><span>ASH / IDEA PARTNER</span><b>CONTEXT LOADED</b></header><p>You don&rsquo;t actually have two ideas here.</p><p>You have one decision engine: detect the competitor gap, then route it to paid ads when intent is immediate or SEO when the opportunity compounds.</p><div><span>REEL</span><span>12 SOURCES</span><span>7 NOTES</span><span>YOUR RULES</span></div></div>
        </section>

        <section id="build" ref={sectionRef(4)} className="fb-chapter fb-build">
          <div className="fb-copy"><span className="fb-eyebrow">05 / THE THOUGHT BECOMES WORK</span><h2>Not another<br />summary.<br /><em>A starting line.</em></h2><p>Choose the shape. Fartbrain produces the context-rich package another model, agent or developer can actually execute.</p></div>
          <div className="fb-build-surface"><header>COMPETITOR SIGNAL ENGINE <span>READY TO BUILD</span></header>{["MVP product brief","System architecture","Research dossier","Implementation prompt","Reusable skill"].map((x,i)=><div key={x}><span>{String(i+1).padStart(2,"0")}</span><b>{x}</b><small>{i===0?"12 sections":"generated from this idea"}</small></div>)}<button onClick={run}>INSPECT BUILD PACKAGE ↗</button></div>
        </section>

        <section id="ash" ref={sectionRef(5)} className="fb-chapter fb-final">
          <span className="fb-eyebrow">06 / YOUR IDEAS WERE NEVER TOO SMALL</span><h2>The good ones<br />just needed<br /><em>somewhere to go.</em></h2><p>Capture the thought. Keep the source. Find the connections. Build the thing.</p><button className="fb-primary" onClick={() => onEnter?.()}>Start your brain <span>↗</span></button>
          <footer><a href="#capture">FARTBRAIN.APP</a><span>PRIVATE BY DEFAULT</span><span><Link to="/privacy">PRIVACY</Link> · <Link to="/terms">TERMS</Link></span></footer>
        </section>
      </main>
