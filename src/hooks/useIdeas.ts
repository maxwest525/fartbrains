...
import { deliverToFolderWebhook } from "@/hooks/useFolderWebhooks";
...
export function useCreateIdea() {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      if (row?.id) {
        // Forward to the folder's webhook, if its owner configured one.
        deliverToFolderWebhook(row.id, row.folder_id ?? null);
...
export function useUpdateIdea() {
  ...
  return useMutation({
    ...
    onSuccess: (_d, vars) => {
      ...
      qc.invalidateQueries({ queryKey: ["folder-counts"] });
      qc.invalidateQueries({ queryKey: ["folder-previews"] });
      // Moving an idea into a folder counts as it landing there.
      if ("folder_id" in vars.patch) {
        deliverToFolderWebhook(vars.id, vars.patch.folder_id ?? null);
      }
    },
    onError: (e: Error) => toast.error(e.message),
  });
